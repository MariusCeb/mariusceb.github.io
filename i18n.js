...
const TRANSLATIONS = {
  ...
  en: {
    ...
    'sr.f2.dsc':  'On mobile, the desktop nav is replaced by an animated hamburger button that triggers a full-screen overlay menu with staggered link entrance animations. A persistent floating bar at the bottom of the screen shows the salon\'s phone number and a "Prenota" button, one tap to call and always visible. This was the most impactful UX addition for a physical-service business.',
    ...
    'fw.what.p2': 'The widget ships as a single IIFE bundle: no framework, no dependencies; it loads via a script tag with a license key attribute, a bookmarklet, or a Chrome extension. On activation it mounts a floating control panel with six element groups (H1, H2, H3, H4–6, Body, Buttons, Nav), full typography sliders (size, line-height, letter-spacing, word-spacing), a color picker, variable font axes (stretch, italic), custom CSS selectors, and lazy-loaded font previews for 310+ Google Fonts across 6 categories. Every change is applied live via injected style tags using !important to override host page styles.',
  ...
  it: {
    ...
    'about.bio.label':  'Bio',
    'about.bio.p1':     '<strong>Sono Ceban Marius</strong>, web developer e designer. Costruisco esperienze digitali pulite, con cura per i dettagli: la spaziatura, il movimento, la sensazione quando le usi.',
    ...
    'about.bio.p3':     'Quando non costruisco cose studio nuovi pattern, sperimento con le interfacce e cerco di capire dove va il web.',
    ...
    'about.proc1.ttl':  'Design',
    'about.proc1.dsc':  'Wireframing, prototipazione e definizione del linguaggio visivo. Ogni vincolo è una scelta.',
    ...
    'us.why.p2':  'UniShare centralizza tutto in uno spazio autenticato. <strong>Il modello delle classi con codice invito rispecchia il funzionamento reale dei corsi universitari</strong>: un professore crea la stanza, gli studenti entrano, e i materiali fluiscono in entrambe le direzioni. L\'abbonamento aggiunge la logica economica: come una piattaforma reale si sostiene.',
    ...
    'fl.why.p2':  'La dimensione locale era fondamentale. La cultura del vino italiano è intensamente regionale: un Sangiovese di Romagna della Cantina San Patrignano è una raccomandazione completamente diversa da un Chianti generico. <strong>Nominare produttori DOC/DOCG specifici e indicare dove si trovano nelle vicinanze rende l\'output effettivamente utilizzabile</strong>, non solo educativo. Un abbinamento su cui puoi agire stasera vale molto di più di uno che non puoi applicare.',
    'fl.why.p3':  'Il modello freemium (3 query gratuite, poi €9,99/mese via Stripe) non è stato un ripensamento. Abbassa la barriera per provare, lascia che la qualità dell\'output AI parli da sola e crea un momento di conversione naturale. Costruire un flusso di pagamento reale, non un mockup, è stato tanto un apprendimento sull\'architettura del prodotto quanto sulla funzionalità in sé.',
    'fl.f1.ttl':  'Tipografia e Sistema di Design',
    'fl.f1.dsc':  'L\'identità visiva di Flavory parte da una scelta tipografica precisa. Playfair Display serve come typeface display principale: i suoi tratti spessi-sottili ad alto contrasto e il carattere editoriale comunicano la stessa sensibilità di un\'etichetta di vino premium o di un menu di ristorante fine. La palette è altrettanto contenuta: cremisi profondo (#b22222) come unico accento su uno sfondo crema quasi bianco (#fff2f2). Questo calore quasi monocromatico attraversa ogni superficie interattiva: il cursore personalizzato, l\'highlight della selezione, il pulsante login, lo sfondo CTA, gli stati hover e l\'enfasi inline nel testo. Tutto parla la stessa lingua, come se ogni elemento fosse stato pensato insieme. Le sezioni marketing confermano questo registro: la lista "Perché Flavory", la sezione scura "Tre passi" e il footer CTA scuro parlano tutti la stessa lingua.',
    ...
    'fn.why.p2':  '<strong>Fontara contrae quel ciclo.</strong> Carica qualsiasi Google Font in millisecondi, lo renderizza in contesti di layout reali, non Lorem Ipsum su una pagina bianca, e ti permette di vedere come si comporta l\'abbinamento a scala reale: titoli compatti, testo corpo lungo, didascalie, etichette UI. I suggerimenti curati aiutano a restringere la scelta: scegli un font titolo e il selettore corpo mostra subito i font che ci stanno bene insieme.',
    'fn.why.p3':  'L\'URL condivisibile non è un\'aggiunta di contorno. Un abbinamento tipografico è utile solo se puoi condividerlo, con un collaboratore, un cliente o il te futuro. Codificare tutto lo stato in un hash base64 compatto significa nessun database, nessun account, nessuna logica server. Solo un link che funziona.',
    ...
    'fw.why.p2':  '<strong>Fontara Widget chiude completamente quel divario.</strong> Poiché si inietta direttamente in qualsiasi sito web live, ogni decisione tipografica avviene su contenuti reali: le lunghezze di riga, gli spazi, il layout vero del tuo prodotto. Il widget punta agli elementi già presenti nella pagina (h1, p, nav a, button) e li sovrascrive in place, così ciò che vedi nel pannello è ciò che pubblicherai.',
